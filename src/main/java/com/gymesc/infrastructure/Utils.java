...
public class Utils {
    ...
    public static String validatePassword(String password) {
        if (password == null || password.length() < 8) {
            return I18N.getInstance().getMessage("validator.message.password.min.size");
        }
        if (!LOWERCASE_PATTERN.matcher(password).find()) {
            return I18N.getInstance().getMessage("validator.message.password.need.lowercase");
        }
        if (!UPPERCASE_PATTERN.matcher(password).find()) {
            return I18N.getInstance().getMessage("validator.message.password.need.uppercase");
        }
        if (!DIGIT_PATTERN.matcher(password).find()) {
            return I18N.getInstance().getMessage("validator.message.password.need.number");
        }
        if (!SPECIAL_CHARACTER_PATTERN.matcher(password).find()) {
            return I18N.getInstance().getMessage("validator.message.password.need.special.char");
        }
